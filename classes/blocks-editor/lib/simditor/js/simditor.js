      ...
      return function(e, file, result) {
        ...
        if (result.success === false) {
          ...
          if (typeof img_path === 'string') {
            try {
              if (img_path.indexOf(location.origin) === 0) {
                img_path = img_path.replace(location.origin, '');
              } else if (/^https?:\/\//.test(img_path)) {
                var _u = new URL(img_path);
                img_path = (_u.pathname || '') + (_u.search || '') + (_u.hash || '');
              }
            } catch (e) {
        ...
        if (_this.popover.active) {
          ...
          return _this.popover.srcEl.val(img_path || result.file_path);
        }
      };
    })(this));
